<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>登录密码:</label>
        <input type="text" placeholder="请输入登陆密码" v-model="password" name="password" v-validate="{required:true,regex:/^[0-9A-Za-z]{8,20}$/}" :class="{invalid:errors.has('password')}">
      ...
      <div class="controls">
        <input type="checkbox"  v-model="agree" name="agree" v-validate="{required:true,'agrees':true}" :class="{invalid:errors.has('agree')}">
  ...
  export default {
    ...
    methods:{
      ...
      async getCode(){
        try {
          const {phone} = this
          phone && (await this.$store.dispatch('getCode',this.phone))  //phone存在才执行
          // 将组件中code属性值变为仓库验证码（直接展示验证码）
