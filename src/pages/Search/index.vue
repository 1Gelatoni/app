...
export default {
  ...
  data() {
    return {
      ...
      searchParams: {
        ...
        "categoryName": "", //分类名字
        "keyword": "",     //关键字
        "order": "1:desc",      //排序 初始值：综合降序
        "pageNo": 1,      //分页器 当前第几页
        "pageSize": 3,    //每一页展示数据的多少
        "props": [],      //商品售卖属性
        "trademark": ""   //品牌
      }
    }
  ...
  mounted() {
    // this.$store.dispatch("getSearchInfo",{})  //这里search只能发送一次请求
    this.getData()
  },
  computed: {
    /* ...mapState({
  ...
  methods: {
    // 向服务器发起请求获取search模块数据（根据参数不同返回不同的数组进行展示）
    // 把请求封装为函数 多次调用
    getData() {
    ...
    removeCategoryName(){
      ...
      // 相应的categoryId置空 带给服务器的参数可有可无时 可将参数设为undefined 该参数就不会传给服务器
      ...
      this.getData()
      // 修改地址栏：进行路由跳转
      // console.log(this.$route.params ); //空对象布尔值为真
